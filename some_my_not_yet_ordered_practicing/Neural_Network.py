...
x = tf.placeholder("float", [None, n_input]) #None样本个数不确定, n_input=784
y = tf.placeholder("float", [None, n_classes]) #None样本个数不确定, n_class=10
...
for epoch in range(training_epochs):
    ...
    for i in range(total_batch):
        ...
        feeds = {x: batch_xs, y: batch_ys} # batch填充值
